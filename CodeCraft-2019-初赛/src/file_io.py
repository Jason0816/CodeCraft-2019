...
def read_cars(car_path):
    ...
    with open(car_path, 'r') as f:
        ...
        for data in datas:
            ...
            cars[int(data[0])] = {'from': int(data[1]), 'to': int(data[2]), 'speed': int(data[3]), 'time': int(data[4])}
    return cars

def read_roads(road_path):
    """ 读取道路信息
        file_path: roads.txt路径
        返回值: dict roads{}
    """
    roads = {}
    with open(road_path, 'r') as f:
        ...
        for data in datas:
            ...
            roads[int(data[0])] = {'length': int(data[1]), 'speed': int(data[2]), 'channel': int(data[3]), 'from': int(data[4]), 'to': int(data[5]), 'isDuplex': bool(int(data[6]))}         
    return roads

def read_cross(cross_path):
    """ 读取路口信息
        file_path: cross.txt路径
        返回值: dict cross{}
    """
    cross = {}
...
#         用以主函数调用
# ==============================================================
def read_input(car_path, road_path, cross_path):
